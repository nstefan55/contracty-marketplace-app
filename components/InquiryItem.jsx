...
export function InquiryItem({ inq, session, isContractor }) {
  const id = inq._id.toString();
  const [status, setStatus] = useState(inq.status);
  const [pending, setPending] = useState(false);

  const { setUnreadCount } = useInquiryContext();
  ...
  async function handleToggleRead() {
    ...
    try {
      ...
      setUnreadCount((prevCount) =>
        next === "read" ? prevCount - 1 : prevCount + 1,
      );
      ...
      <CardContent className="pt-0 gap-2 flex">
        {isRecipient && (
          <>
            ...
            <button
              className="text-sm text-white rounded-2xl font-bold bg-red-500 hover:bg-red-500/80 p-2 mt-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
