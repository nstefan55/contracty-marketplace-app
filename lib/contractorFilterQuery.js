function safeRegex(input) {
  const s = String(input).slice(0, 64); // Limit length to prevent ReDoS
  const escaped = s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"); // Escape regex special chars
  return new RegExp(escaped, "i"); // Case-insensitive search
}
...
export default function contractorFilterQuery(sp) {
  ...
  if (sp.serviceArea) {
    const rx = safeRegex(sp.serviceArea);
  ...
  if (sp["serviceArea.address"]) {
    query["serviceArea.address"] = safeRegex(sp["serviceArea.address"]);
  }

  if (sp["serviceArea.postcode"]) {
    query["serviceArea.postcode"] = safeRegex(sp["serviceArea.postcode"]);
  }
