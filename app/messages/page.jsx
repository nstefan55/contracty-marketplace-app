                ...
                <Card key={id} className="hover:shadow-sm transition-shadow">
                  <CardContent className="pt-5 space-y-3">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="font-medium text-dark-text">{contactName}</p>
                        <p className="text-xs text-muted-foreground">{subLine}</p>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-xs text-muted-foreground">{date}</span>
                        <Badge variant={STATUS_VARIANT[inq.status] ?? "default"}>
                          {inq.status}
                        </Badge>
                      </div>
                    </div>
                    ...
                    <p className="text-sm text-dark-text border-t pt-3">{inq.description}</p>

                    {inq.replies?.length > 0 && (
                      <div className="border-t pt-3 space-y-2">
                        <p className="text-xs font-medium text-muted-foreground">
                          {inq.replies.length} {inq.replies.length === 1 ? "reply" : "replies"}
                        </p>
